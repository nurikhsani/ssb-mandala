...
<div class="col-md-12 col-sm-12  ">
                    ...
                    <tbody>
                        ...
                        foreach ($siswa as $t) { ?>
                            <tr>
                                ...
                                <td><?= $t['alamat'] ?></td>
                                <td><?= $t['usia'] ?></td>
                                <td><?php if ($t['sudah_bayar'] == 1) {
                                        echo 'Sudah Bayar';
                                    } else { ?>
                                        <a href="<?= base_url('siswa/status/1/' . $t['id_siswa']); ?>" class="text-danger" onclick="return confirm('Yakin Akan Ubah Status?')">Belum Bayar</a>
                                    <?php } ?>
                                </td>
                                <td>
                                    <a href="<?= base_url('siswa/del_siswa/' . $t['id_user']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Akan Dihapus?')"><i class="fa fa-trash"></i></a>
    ...
    <script>
        $("#btnCari").click(function() {
            ...
            window.location.href = "<?php echo base_url() ?>siswa/belum_bayar?usia=" + cari_usia;
        });

        $("#btnReset").click(function() {
            $("#cari_usia").val("0");
            window.location.href = "/ssb-mandala/siswa/belum_bayar";
        });
    </script>
</div>
</div>
